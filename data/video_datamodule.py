...
from data import SpyGlass2dDataset, SpyGlassVideoDataset
...
class SpyGlassDataModule(LightningDataModule):
    ...
    def __init__(self, input_root: str, channels: int, time_depth: int, x_size: int, y_size: int,
                 medical_data_csv_path: str=None, mode: str='2d',
    ...
    def init_transforms(self):
        assert self.mode in ['2d', 'video'], "mode must be '2d' or 'video'"
        if self.mode=='2d':
    ...
    def init_full_dataset(self, train=True, transform=None):
        assert self.mode in ['2d', 'video'], "mode must be '2d' or 'video'"
        if self.mode=='2d':
            return SpyGlass2dDataset(self.input_root, self.medical_data_csv_path, 
            ...
            return SpyGlassVideoDataset(self.input_root, self.channels, self.time_depth,
                                        ...
                                        self.medical_data_csv_path, train=train, transform=transform)
